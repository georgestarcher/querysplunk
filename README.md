...
Splunk MCP Server and querysplunk are complementary. An AI assistant can use
MCP for interactive discovery and quick questions, then use querysplunk when a
search should be validated, saved, reviewed, automated, or diagnosed in more
detail.

| Splunk MCP Server | querysplunk |
| --- | --- |
| Exposes native tools for searches, indexes, metadata, users, saved searches, and knowledge objects. | Runs SPL from text or YAML with explicit app, dispatch, result, safety, and diagnostic settings. |
| Applies server-managed RBAC, tool controls, row limits, rate limits, timeouts, and SPL guardrails. | Applies local safety validation and preserves normal Splunk Search API behavior. |
| Uses an MCP-encrypted token generated by the Splunk MCP Server app. | Uses a normal Splunk token or username and password. |
| Can connect to the management endpoint on port `8089` or, when provided by Splunk Cloud, through the Splunk Web proxy on port `443`. This can fit networks that do not allow clients to reach the management port directly. | Must reach the Splunk management REST API, normally `https://<host>:8089`. Splunk Cloud REST API access and its IP allowlist must permit the client. |
| Best for interactive exploration and supported MCP operations. | Best for repeatable YAML libraries, CI, long-running jobs, saved output, SID recovery, and bounded `search.log` diagnostics. |

MCP command restrictions are controlled by the installed Splunk MCP Server
version and configuration. For example, Splunk MCP Server 1.2.1 can reject a
`rest` command submitted through its general search tool even though the same
read-only SPL is valid through the Splunk Search API. Prefer a dedicated MCP
tool when one covers the operation, such as index or metadata discovery. Use
querysplunk for valid searches that need broader Search API behavior, including
the REST-based health examples in this repository.

A practical assistant workflow is to explore through MCP, refine the SPL, and
save useful recurring searches as querysplunk YAML. Keeping both available lets
the assistant choose the supported network path and execution controls for the
task instead of repeatedly retrying an incompatible command.
