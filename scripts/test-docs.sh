...
grep -F 'add_orphan_field=true' examples/rest/orphaned-scheduled-searches.yml >/dev/null || fail "orphaned-search inspection does not request Splunk orphan status"
grep -F 'search="orphan=1" search="is_scheduled=1" count=100' examples/rest/orphaned-scheduled-searches.yml >/dev/null || fail "orphaned-search inspection does not filter before its finite result bound"
grep -F '| where orphan=1 AND is_scheduled=1' examples/rest/orphaned-scheduled-searches.yml >/dev/null || fail "orphaned-search inspection does not select scheduled orphaned searches"
grep -F '| rename eai:acl.app AS app eai:acl.owner AS owner' examples/rest/orphaned-scheduled-searches.yml >/dev/null || fail "orphaned-search inspection does not expose simple app and owner fields"
...
for config in examples/rest/system-messages.yml examples/rest/saved-search-definition.yml examples/rest/orphaned-scheduled-searches.yml examples/rest/macro-definitions.yml examples/rest/lookup-definitions.yml; do
